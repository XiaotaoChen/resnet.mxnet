...
import mxnet as mx
from .GDRQ_quantization_int8 import GDRQ_quant_conv
from .GDRQ_quantization_int8 import GDRQ_quant_fc
...
def Conv(data, num_filter=1, channel=1, kernel=(1, 1), stride=(1, 1), pad=(0, 0), num_group=1, name='', suffix='',
    ...
    conv = GDRQ_quant_conv(name='%s%s_conv2d' % (name, suffix), data=data, num_filter=num_filter, kernel=kernel, num_group=num_group,
...
def mobilenet_int8_gdrq(num_classes, alpha=1, resolution=224, 
    ...
    fc = GDRQ_quant_fc(name='fc', data=flatten, num_hidden=num_classes, 
