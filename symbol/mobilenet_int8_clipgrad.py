...
import mxnet as mx
from .clip_grad_quantization_int8 import clipgrad_quant_conv
from .clip_grad_quantization_int8 import clipgrad_quant_fc
...
def Conv(data, num_filter=1, channel=1, kernel=(1, 1), stride=(1, 1), pad=(0, 0), num_group=1, name='', suffix='',
    ...
    conv = clipgrad_quant_conv(name='%s%s_conv2d' % (name, suffix), data=data, num_filter=num_filter, kernel=kernel, num_group=num_group,
...
def mobilenet_int8_clipgrad(num_classes, alpha=1, resolution=224, 
    ...
    conv_1 = Conv(data, num_filter=depth, channel=3, kernel=(3, 3), pad=(1, 1), stride=(2, 2), name="conv_1",
                  quant_mod=quant_mod, delay_quant=delay_quant, is_weight_perchannel=is_weight_perchannel)  # 224/112

    depth = base  # 32*alpha
    conv_2_dw = Conv(conv_1, num_group=depth, num_filter=depth, channel=depth, kernel=(3, 3), pad=(1, 1), stride=(1, 1),
                     name="conv_2_dw", 
                     quant_mod=quant_mod, delay_quant=delay_quant, is_weight_perchannel=is_weight_perchannel)  # 112/112
    ...
    conv_3_dw = Conv(conv_2, num_group=depth, num_filter=depth, channel=depth, kernel=(3, 3), pad=(1, 1), stride=(2, 2),
                     name="conv_3_dw",
                     quant_mod=quant_mod, delay_quant=delay_quant, is_weight_perchannel=is_weight_perchannel)  # 112/56
    conv_3 = Conv(conv_3_dw, num_filter=depth * 2, channel=depth, kernel=(1, 1), pad=(0, 0), stride=(1, 1), name="conv_3",
                  quant_mod=quant_mod, delay_quant=delay_quant, is_weight_perchannel=is_weight_perchannel)  # 56/56
    ...
    conv_5_dw = Conv(conv_4, num_group=depth, num_filter=depth, channel=depth, kernel=(3, 3), pad=(1, 1), stride=(2, 2),
                     name="conv_5_dw",
                     quant_mod=quant_mod, delay_quant=delay_quant, is_weight_perchannel=is_weight_perchannel)  # 56/28
    conv_5 = Conv(conv_5_dw, num_filter=depth * 2, channel=depth, kernel=(1, 1), pad=(0, 0), stride=(1, 1), name="conv_5",
                  quant_mod=quant_mod, delay_quant=delay_quant, is_weight_perchannel=is_weight_perchannel)  # 28/28
    ...
    conv_7_dw = Conv(conv_6, num_group=depth, num_filter=depth, channel=depth, kernel=(3, 3), pad=(1, 1), stride=(2, 2),
                     name="conv_7_dw",
                     quant_mod=quant_mod, delay_quant=delay_quant, is_weight_perchannel=is_weight_perchannel)  # 28/14
    conv_7 = Conv(conv_7_dw, num_filter=depth * 2, channel=depth, kernel=(1, 1), pad=(0, 0), stride=(1, 1), name="conv_7",
                  quant_mod=quant_mod, delay_quant=delay_quant, is_weight_perchannel=is_weight_perchannel)  # 14/14
    ...
    conv_13_dw = Conv(conv_12, num_group=depth, num_filter=depth, channel=depth, kernel=(3, 3), pad=(1, 1), stride=(2, 2),
                      name="conv_13_dw",
                      quant_mod=quant_mod, delay_quant=delay_quant, is_weight_perchannel=is_weight_perchannel)  # 14/7
    conv_13 = Conv(conv_13_dw, num_filter=depth * 2, channel=depth, kernel=(1, 1), pad=(0, 0), stride=(1, 1), name="conv_13",
                   quant_mod=quant_mod, delay_quant=delay_quant, is_weight_perchannel=is_weight_perchannel)  # 7/7
    ...
    fc = clipgrad_quant_fc(name='fc', data=flatten, num_hidden=num_classes, 
